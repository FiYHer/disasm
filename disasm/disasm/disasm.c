...
ulong Disasm(unsigned char *src,	//需要翻译成汇编代码的十六进制文本
	ulong srcsize,					//十六进制文本的长度
	ulong srcip,					//
  t_disasm *disasm,					//接收汇编指信息
	int disasmmode)					//反汇编模式
{
	int i, j, isprefix, is3dnow, repeated, operand, mnemosize, arg;
	ulong u, code;
	int lockprefix;                      // Non-zero if lock prefix present【如果存在锁定前缀，就为非0】
	int repprefix;                       // REPxxx prefix or 0【如果存在重复前缀，就为非0】
	int cxsize;						     // 长度
	unsigned char name[TEXTLEN], *pname;
	const t_cmddata *pd, *pdan;
	// Prepare disassembler variables and initialize structure disasm.
	// 准备反汇编变量并初始化结构disasm
	datasize = addrsize = 4;                 // 32-bit code and data segments only!【仅限32位代码和数据段！】
	segprefix = SEG_UNDEF;				     // 段重载前缀
	hasrm = hassib = 0;
	dispsize = immsize = 0;
	lockprefix = 0;
	repprefix = 0;
	...
	da->ip = srcip;
	da->comment[0] = '\0';
	da->cmdtype = C_BAD;					//无法识别的命令
	da->nprefix = 0;
	da->memtype = DEC_UNKNOWN;				//未知类型
	da->indexed = 0;
	da->jmpconst = 0;
	da->jmptable = 0;
	da->adrconst = 0;
	...
	da->fixupsize = 0;
	da->warnings = 0;						// 警告
	da->error = DAE_NOERR;					// 没有发生错误
	mode = disasmmode;						// No need to use register contents【无需使用寄存器内容】
	...
	u = 0; repeated = 0;
	while (size > 0)						 //十六进制转储长度不为空
	{
		isprefix = 1;                        // Assume that there is some prefix【假设有一个前缀】
		switch (*cmd)						 //	A-2单字节表的判断
		{
