	...
	int nDisasmMode)					//反汇编模式
{
	...
	ulong u;
	ulong code;
	int nLockPrefix;                     // Non-zero if lock prefix present【当前的锁定前缀】
	int nRepPrefix;                      // REPxxx prefix or 0【当前的重复前缀】
	int cxsize;						     // 长度
	...
	/* Correct 80x86 command may theoretically contain up to 4 prefixes belonging
	 ...
	 为了维持这一限制，如果disasm（）检测同一组中的第二个前缀，它首先刷新作为伪命令的序列前缀*/
	u = 0;									 // 前缀的数量为0
	nIsRepeated = 0;						 // 没有重复前缀
	while (g_ulHexLength > 0)				 //十六进制转储长度不为空
	{
		nIsPrefix = 1;                       // Assume that there is some prefix【假设有一个前缀】
		switch (*g_uszCmd)						 //	A-2单字节表的判断，先判断段前缀
		{
		case 0x26: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_ES;
				   else nIsRepeated = 1; break;
		case 0x2E: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_CS;
				   else nIsRepeated = 1; break;
		case 0x36: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_SS;
				   else nIsRepeated = 1; break;
		case 0x3E: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_DS;
				   else nIsRepeated = 1; break;
		case 0x64: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_FS;
				   else nIsRepeated = 1; break;
		case 0x65: if (g_nSegPrefix == SEG_UNDEF) g_nSegPrefix = SEG_GS;
				   else nIsRepeated = 1; break;
		case 0x66: if (g_ulDataSize == 4) g_ulDataSize = 2;			//操作数大小重载前缀
				   else nIsRepeated = 1; break;
		case 0x67: if (g_ulAddrSize == 4) g_ulAddrSize = 2;			//地址大小重载前缀
				   else nIsRepeated = 1; break;
		case 0xF0: if (nLockPrefix == 0) nLockPrefix = 0xF0;	//锁定前缀
				   else nIsRepeated = 1; break;
		case 0xF2: if (nRepPrefix == 0) nRepPrefix = 0xF2;	//重复前缀
				   else nIsRepeated = 1; break;
		case 0xF3: if (nRepPrefix == 0) nRepPrefix = 0xF3;	//重复前缀
				   else nIsRepeated = 1; break;
		default: nIsPrefix = 0; break;
		};
		if (nIsPrefix == 0 || nIsRepeated != 0)	 // No more prefixes or duplicated prefix【不再有前缀或重复的前缀】
			break;
	...
	if (nIsRepeated)
	{
		if (g_nDisasmMode >= DISASM_FILE)		// 当前的模式是反汇编模式
		{
			...
			g_nResultLen += sprintf(g_stDisasmInfo->uszResult + g_nResultLen, "PREFIX %s:", pname);
			if (g_nLowerCase) strlwr(g_stDisasmInfo->uszResult);
			if (g_nExtraPrefix == 0) strcpy(g_stDisasmInfo->uszComment, "Superfluous prefix");
		};
		g_stDisasmInfo->nWarnings |= DAW_PREFIX;
		if (nLockPrefix) g_stDisasmInfo->nWarnings |= DAW_LOCK;
		g_stDisasmInfo->nCmdType = C_RARE;
		return 1;                          // Any prefix is 1 byte long【任何前缀都1Byte长度】
	};
	/* If lock prefix available, display it and forget, because it has no
	 influence on decoding of rest of the command.*/
	 /*【如果有锁定前缀，则不管，因为它没有对命令其余部分解码的影响】*/
	if (nLockPrefix != 0)
	{
		if (g_nDisasmMode >= DISASM_FILE) 
			g_nResultLen += sprintf(g_stDisasmInfo->uszResult + g_nResultLen, "LOCK ");//格式化锁定前缀
		g_stDisasmInfo->nWarnings |= DAW_LOCK;		   // 警告
	};
	/* Fetch (if available) first 3 bytes of the command, add repeat prefix and
	 find command in the command table.*/
	 /*【获取（如果可用）命令的前3个字节，添加重复前缀和在命令表中查找命令】*/
	code = 0;
	if (g_ulHexLength > 0) *(((unsigned char *)&code) + 0) = g_uszCmd[0];
	if (g_ulHexLength > 1) *(((unsigned char *)&code) + 1) = g_uszCmd[1];
	if (g_ulHexLength > 2) *(((unsigned char *)&code) + 2) = g_uszCmd[2];
	if (nRepPrefix != 0)							// RER/REPE/REPNE is considered to be【认为是F3H】
		code = (code << 8) | nRepPrefix;			// part of command.【部分命令】
	if (g_nDecodeVxD && (code & 0xFFFF) == 0x20CD)
		pd = &cstVxDCmd;							// Decode VxD call (Win95/98)【解码vxd调用】
	else
	{
		for (pd = cstCmdData; pd->ulMask != 0; pd++) //这个循环干什么的？？
		{
			if (((code^pd->ulCode) & pd->ulMask) != 0)//^二进制【位相同为0，不相同为1】
				continue;
	...
	if ((pd->ucType & C_TYPEMASK) == C_NOW)// 3DNow! commands require additional search.【3DNow!命令需要额外的搜索】
	{
		...
		else
		{
			for (; pd->ulMask != 0; pd++)
			{
				if (((code^pd->ulCode) & pd->ulMask) != 0) continue;
				if (((uchar *)&(pd->ulCode))[2] == j) break;
			};
		};
	};
	if (pd->ulMask == 0) // Command not found【找不到命令】
	{
		g_stDisasmInfo->nCmdType = C_BAD;
		if (g_ulHexLength < 2) g_stDisasmInfo->nError = DAE_CROSS;
		else g_stDisasmInfo->nError = DAE_BADCMD;
	}
	else // Command recognized, decode it【命令识别，解码】
	{
		g_stDisasmInfo->nCmdType = pd->ucType;
		cxsize = g_ulDataSize;					    // Default size of ECX used as counter【用作计数器的ECX的默认大小】
		...
		if (g_uszCmd[0] == 0x44 || g_uszCmd[0] == 0x4C ||
		...
		{
			if (pd->uszName[0] == '&')
				mnemosize = g_ulDataSize;
			else if (pd->uszName[0] == '$')
				mnemosize = g_ulAddrSize;
			else 
				mnemosize = 0;
			if (mnemosize != 0)
			{
				for (i = 0, j = 1; pd->uszName[j] != '\0'; j++)
				{
					...
					else if (pd->uszName[j] == '*') //Substitute by 'W', 'D' or none 【用“w”、“d”或“none”替换】
					{
						if (mnemosize == 4 && g_nSizeSens != 2)
							name[i++] = 'D';
						else if (mnemosize != 4 && g_nSizeSens != 0)
							name[i++] = 'W';
					}
					else
						name[i++] = pd->uszName[j];
			...
			if (nRepPrefix != 0 && g_nTabArguments)
			{
				...
				g_nResultLen += sprintf(g_stDisasmInfo->uszResult + g_nResultLen, "%s", name);
			if (g_nLowerCase)
				strlwr(g_stDisasmInfo->uszResult);
		};
		/* Decode operands (explicit - encoded in command, implicit - present in
		 ...
		 are allowed.*/
		 /*【解码操作数（显式-在命令中编码，隐式-在mmemonic或假定-由命令使用或修改）。假定操作数
			必须保留在所有显式和隐式操作数之后。最多3个操作是允许的。】*/
		for (nOperandCount = 0; nOperandCount < 3; nOperandCount++)
		{
			...
			/* If command contains both source and destination, one usually must not
			 ...
			 /*【如果命令同时包含源和目标，则通常不能将目标解码为注释，因为它将在
			 下一步。global addcomment会处理这个问题。解码程序，但是，可以忽略此标志。】*/
			...
			if ((g_nDisasmMode >= DISASM_FILE) && arg < PSEUDOOP)
			{
				if (nOperandCount == 0)
				{
				...
				{
					g_stDisasmInfo->uszResult[g_nResultLen++] = ',';
					if (g_nExtraSpace) g_stDisasmInfo->uszResult[g_nResultLen++] = ' ';
				};
			};
			// Decode, analyse and comment next operand of the command.【解码、分析并注释命令的下一个操作数。】
			switch (arg)
			{
			...
			case RCM:                      // Integer register in command byte【命令字节中的整数寄存器】
				DecodeRG(g_uszCmd[0], g_ulDataSize, RCM); break;
			case RG4:                      // Integer 4-byte register in Reg field【REG字段中的整数4字节寄存器】
				if (g_ulHexLength < 2) g_stDisasmInfo->nError = DAE_CROSS;
				else DecodeRG(g_uszCmd[1] >> 3, 4, RG4);
				g_nModRM = 1; break;
			case RAC:                      // Accumulator (AL/AX/EAX, implicit)
				DecodeRG(REG_EAX, g_ulDataSize, RAC); break;
			...
			case RST:                      // FPU register (ST(i)) in command byte
				DecodeST(g_uszCmd[0], 0); break;
			case RMX:                      // MMX register MMx
				if (g_ulHexLength < 2) g_stDisasmInfo->nError = DAE_CROSS;
				else DecodeMX(g_uszCmd[1] >> 3);
				g_nModRM = 1; break;
			case R3D:                      // 3DNow! register MMx
				if (g_ulHexLength < 2) g_stDisasmInfo->nError = DAE_CROSS;
				else DecodeNR(g_uszCmd[1] >> 3);
				g_nModRM = 1; break;
			case MRG:                      // Memory/register in ModRM byte
			case MRJ:                      // Memory/reg in ModRM as JUMP target
			case MR1:                      // 1-byte memory/register in ModRM byte
			...
			case MMS:                      // Memory in ModRM byte (as SEG:OFFS)
				DecodeMR(arg);
				g_stDisasmInfo->nWarnings |= DAW_FARADDR; break;
			case RR4:                      // 4-byte memory/register (register only)
			case RR8:                      // 8-byte MMX register only in ModRM
			case RRD:                      // 8-byte memory/3DNow! (register only)
				if ((g_uszCmd[1] & 0xC0) != 0xC0) g_nSoftError = DAE_REGISTER;
				DecodeMR(arg); break;
			...
			case IMU:                      // Immediate unsigned data (8 or 16/32)
				if ((pd->ucBits & SS) != 0 && (*g_uszCmd & 0x02) != 0)
					DecodeIM(1, g_ulDataSize, arg);
				else
					DecodeIM(g_ulDataSize, 0, arg);
				break;
			case VXD:                      // VxD service (32-bit only)
				DecodeVX(); break;
			case IMX:                      // Immediate sign-extendable byte
				DecodeIM(1, g_ulDataSize, arg); break;
			...
			case IM2:                      // Immediate word (ENTER/RET)
				DecodeIM(2, 0, arg);
				if ((g_stDisasmInfo->ulImmConst & 0x03) != 0) g_stDisasmInfo->nWarnings |= DAW_STACK;
				break;
			case IMA:                      // Immediate absolute near data address
				DecodeIA(); break;
			case JOB:                      // Immediate byte offset (for jumps)
				DecodeRJ(1, ulHexAddress + 2); break;
			case JOW:                      // Immediate full offset (for jumps)
				DecodeRJ(g_ulDataSize, ulHexAddress + g_ulDataSize + 1); break;
			case JMF:                      // Immediate absolute far jump/call addr
				DecodeJF();
				g_stDisasmInfo->nWarnings |= DAW_FARADDR; break;
			case SGM:                      // Segment register in ModRM byte
				if (g_ulHexLength < 2) g_stDisasmInfo->nError = DAE_CROSS;
				DecodeSG(g_uszCmd[1] >> 3); g_nModRM = 1; break;
			case SCM:                      // Segment register in command byte
				DecodeSG(g_uszCmd[0] >> 3);
				if ((g_stDisasmInfo->nCmdType & C_TYPEMASK) == C_POP) g_stDisasmInfo->nWarnings |= DAW_SEGMENT;
				break;
			case CRX:                      // Control register CRx
				if ((g_uszCmd[1] & 0xC0) != 0xC0) g_stDisasmInfo->nError = DAE_REGISTER;
				DecodeCR(g_uszCmd[1]); break;
			case DRX:                      // Debug register DRx
				if ((g_uszCmd[1] & 0xC0) != 0xC0) g_stDisasmInfo->nError = DAE_REGISTER;
				DecodeDR(g_uszCmd[1]); break;
			...
			case PAC:                      // Accumulator (AL/AX/EAX, pseudooperand)
				DecodeRG(REG_EAX, g_ulDataSize, PAC); break;
			...
			default:
				g_stDisasmInfo->nError = DAE_INTERN;        // Unknown argument type【未知参数类型】
				break;
			};
		};
		...
		/* Segment prefix and address size prefix are superfluous for command which
		 ...
		 in analysis.*/
		 /*【段前缀和地址大小前缀无法访问内存。如果是这样，请将命令标记为罕见在分析中】*/
		...
		/*【16位寻址在32位程序中很少见。如果是这种情况，请将命令标记为罕见，以帮助分析。】*/
		if (g_ulAddrSize != 4) 
			g_stDisasmInfo->nCmdType |= C_RARE;
	};
	...
	if (nIs3DNow)
	{
		if (g_nImmSize != 0) g_stDisasmInfo->nError = DAE_BADCMD;
		else g_nImmSize = 1;
	};
	...
	{                  
		...
		{
			if (g_nDisasmMode >= DISASM_FILE) g_nDumpLen += sprintf(g_stDisasmInfo->uszDump + g_nDumpLen, "%02X", *g_uszCmd);
			g_uszCmd++; g_ulHexLength--;
		};
		if (g_ulHexLength > 0) 
		{
			if (g_nDisasmMode >= DISASM_FILE) g_nDumpLen += sprintf(g_stDisasmInfo->uszDump + g_nDumpLen, "%02X", *g_uszCmd);
	...
	// Check that command is not a dangerous one.【检查命令是否不危险。】
	if (g_nDisasmMode >= DISASM_DATA)
	{
		for (pdan = cstDangerous; pdan->ulMask != 0; pdan++)
		{
			if (((code^pdan->ulCode) & pdan->ulMask) != 0)
				continue;
	...
	if (g_nDisasmMode >= DISASM_FILE)
	{
		if (g_stDisasmInfo->nError != DAE_NOERR) switch (g_stDisasmInfo->nError)
		{
		case DAE_CROSS:
		...
		else if ((g_stDisasmInfo->nWarnings & DAW_PRIV) != 0 && g_nPrivileged == 0)
			strcpy(g_stDisasmInfo->uszComment, "Privileged command");
		else if ((g_stDisasmInfo->nWarnings & DAW_IO) != 0 && g_nIOCommand == 0)
			strcpy(g_stDisasmInfo->uszComment, "I/O command");
		else if ((g_stDisasmInfo->nWarnings & DAW_FARADDR) != 0 && g_nFarCalls == 0)
		{
			if ((g_stDisasmInfo->nCmdType & C_TYPEMASK) == C_JMP)
				strcpy(g_stDisasmInfo->uszComment, "Far jump");
			else if ((g_stDisasmInfo->nCmdType & C_TYPEMASK) == C_CAL)
				strcpy(g_stDisasmInfo->uszComment, "Far call");
			else if ((g_stDisasmInfo->nCmdType & C_TYPEMASK) == C_RET)
				strcpy(g_stDisasmInfo->uszComment, "Far return");
			;
		}
		else if ((g_stDisasmInfo->nWarnings & DAW_SEGMENT) != 0 && g_nFarCalls == 0)
			strcpy(g_stDisasmInfo->uszComment, "Modification of segment register");
		else if ((g_stDisasmInfo->nWarnings & DAW_SHIFT) != 0 && g_nBadShift == 0)
			strcpy(g_stDisasmInfo->uszComment, "Shift constant out of range 1..31");
		else if ((g_stDisasmInfo->nWarnings & DAW_PREFIX) != 0 && g_nExtraPrefix == 0)
			strcpy(g_stDisasmInfo->uszComment, "Superfluous prefix");
		else if ((g_stDisasmInfo->nWarnings & DAW_LOCK) != 0 && g_nLockedBus == 0)
			strcpy(g_stDisasmInfo->uszComment, "LOCK prefix");
		else if ((g_stDisasmInfo->nWarnings & DAW_STACK) != 0 && g_nStackAlign == 0)
